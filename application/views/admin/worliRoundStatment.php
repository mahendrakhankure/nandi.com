        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-4 offset-md-4">
                    <h3>Instant Worli 1 Live Round Settlement For Round (<span id="roundId"></span>)</h3>
                ...
                <div class="col-md-3" id="userList">
                    ...
                    <!-- <ul id="listOfPlayers" class="list-group"></ul> -->
                </div>
            </div>
            <div class="row">
...
<script>
    ...
    socket.on('resultdeclare', function (d) {
        var res = d.data;
        if (res.market=='WorliRoundStetment') {
            // console.log(res)
            ...
            var wCID = res.customer_id.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '-')
            console.log(wCID)
            if(cArr.includes(wCID)) {
                if(parseInt(res.game)<10){
                    var am = $('#AK_'+wCID).text();
                    if(!am){
                        am = 0;
                    }
                ...
                cArr.push(wCID);
                if(parseInt(res.game)<10){
                    $('#listOfPlayers > tbody:last-child').append('<tr id="'+wCID+'"><td>'+res.userName+'</td><td id="AK_'+wCID+'" class="sortnr">'+res.point+'</td><td id="PATTI_'+wCID+'">0</td></tr>');
                    ...
                    $('#listOfPlayers > tbody:last-child').append('<tr id="'+wCID+'"><td>'+res.userName+'</td><td id="AK_'+wCID+'" class="sortnr">0</td><td id="PATTI_'+wCID+'">'+res.point+'</td></tr>');
            ...
            var rows = $('#listOfPlayers > tbody').children('tr').get(); // creates a JS array of DOM elements
            ...
            for (var i = 0; i < rows.length; i++) {  // .append() will move them for you
                $('#listOfPlayers > tbody').append(rows[i]);
            }


        }else if(res.market=='WorliNewRound'){
...
<script type="text/javascript">
    $('#resWorli').click(function(){
        ...
        if(r=='' || gId==''){
            ...
            $.ajax({
                type: "POST",
                url: base_url+"/a12d4048968d8a6e14c323cff3180c97",
    ...
    $('#buf').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: base_url+"/2e02d8735758cba49091c857f7917fa4",
            data: {status:s},
            success: function (res) {
            }
        });
