        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 offset-md-6">
                    ...
                    <div class="form-group">
                        <label for="videoLink">Deler Video</label>
                        <select class="form-control" id="videoLink" name="videoLink" onchange="setVideo(this.value)">  
                        </select>
                    </div>
                    <div class="form-group">
...
<script type="text/javascript">
    ...
    function setPatti(q){
        ...
        $.ajax({
            type: "POST",
            url: base_url+"/5be215a5f3a02677a1ccbd36b481bd1d",
            ...
            success: function (res) {
                ...
                if(nR.status==200){
                    ...
                    $('#res').val(bPatti);
                    $('#videoLink').empty();
                    var op = '<option>Select Patti</option>';
                    $(nR.data).each(function(l) {
                        op += '<option value="'+nR.data[l]['dealer']+'">'+nR.data[l]['patti']+'</option>';
                    });
                    $('#videoLink').empty().append(op);
                    $('#'+q+'P').css({'background-color': 'orange','color': '#fff'})
                }else{
                    alert(nR.massage)
                    $('#videoLink').empty().append('<option>Select Patti</option>');
                    return false;
                }
            }
        });
    ...
    function setVideo(q){
        var marketId = $('#game_name').val();
        $.ajax({
            type: "POST",
            url: base_url+"/d0766a376187e9be133972bb682c266d",
            data: {marketId:marketId,link:q},
            success: function (res) {
                var nR = JSON.parse(res);
                console.log(nR)
                if(nR.status==200){
            ...
            $(jodi[l]).each(function(j) {
                ...
                if(jodiData.data[n]){
                    ...
                    if((cutAK < jodi[l][j] && (cutAK+9) > jodi[l][j]) || (cutAK+9) == jodi[l][j] || cutAK == jodi[l][j]){
                        ...
                        var cutA = (parseInt(jodiData.data[[n]])*10);
                        tx += '<div style="background-color:#746e71;color:#fff;">'+cutA+'</div>';
                        jT += cutA;
                        if(cutAK+9 == jodi[l][j]){
                            tx += '<div style="background-color:red;color:#fff;">'+jT+'</div>';
    ...
    $('#buf').click(function(){
        ...
        if(bPatti==''){
            ...
            $.ajax({
                type: "POST",
                url: base_url+"/9f38ae5f654182b8b4192eb414be21d9",
                data: {status:s,id:4,bazar:bazar,type:op,bPatti:bPatti},
                success: function (res) {
                    var nR = JSON.parse(res);
                    if(nR.status!=200){
                        $('#buf').click();
