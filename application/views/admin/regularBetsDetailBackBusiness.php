...
<?php
    function get_common_sql($bazar,$game,$game_type){
        ...
        if(in_array($game,$arr)){
            $con=' WHERE result_date BETWEEN "'.$_GET['from'].'" AND "'.$_GET['to'].'" AND game_name= "'.$game.'" AND game_type="'.$game_type.'" AND bazar_name ="'.$bazar.'"';
        } else {
          	$con=' WHERE result_date BETWEEN "'.$_GET['from'].'" AND "'.$_GET['to'].'" AND game_name= "'.$game.'" AND game_type="'.$game_type.'" AND bazar_name ="'.$bazar.'"';
        }
		...
		var userDataTable = $('#userTable').DataTable({
			...
			'processing': true,
	          ...
	          'data': function(data){
	          		...
	          		data.game_name = <?=$game?>;
	          		data.type = "<?=$type?>";
	          		data.result_date = "<?=$_GET['from']?$_GET['from']:date('Y-m-d');?>";
	          		data.to_date = "<?=$_GET['to']?$_GET['to']:date('Y-m-d');?>";
	          }
	      	},
	      	'columns': [
	         	{ data: 'sr' },
