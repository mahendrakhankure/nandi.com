...
//Class Manage_Matkagames extends CI_Controller {
 ...
 //    public function AddAllGameResult($id=''){
	...
	public function regularMarketBetRecords(){
		...
		$con=' INNER JOIN regular_bazar ON regular_bazar_games.bazar_name=regular_bazar.id WHERE regular_bazar_games.result_date="'.date('Y-m-d').'"';
	...
	public function regularTypeRecords($id,$name){
        ...
        $con=' INNER JOIN regular_game_type ON regular_bazar_games.game_name=regular_game_type.id WHERE regular_bazar_games.result_date="'.date('Y-m-d').'" AND regular_bazar_games.bazar_name="'.$id.'"';
	...
	public function tabsData(){
		...
		if($_POST['name']=='JODI'){
			...
			$arr=[];
			$con = ' WHERE game_name IN ("'.implode('","',$gameName).'") AND result_date="'.date('Y-m-d').'" AND bazar_name="'.$_POST['id'].'"';
	...
	public function pnlBetween(){
		...
		if($_POST){
			...
			$feilds='COUNT(id) as id, SUM(point_in_rs) as point, SUM(winning_in_rs) as win';
			...
			$rTI=(int)$data['redTable']['id'];
			$rTB=(int)$data['redTable']['point'];
			$rTW=(int)$data['redTable']['win'];
			$rTG=$rTB-$rTW;
			...
			/*-------------------- Striming Bazar P&L Start -----------------------*/
				...
				$con2 = $con." AND bazar_name IN (SELECT id FROM regular_bazar WHERE bazar_type='Out')";
				...
				$con3 = $con." AND bazar_name IN (SELECT id FROM regular_bazar WHERE bazar_type='InHome')";
			...
			$dome = '<div class="row">
						...
						<div class="col-lg-3 col-md-3 col-xs-6">
							<div class="small-box bg-red">
								<div class="inner">
									<h4 class="text-center">Overall Worli 
									GGR</h4>Total : <span class="bets">'.$wGI.'</span><br>
									Points : <span class="bets">'.$wGB.'</span><br>
									Winning : <span class="bets">'.$wGW.'</span><br>
									GGR : <span class="bets">'.$wGggr.'</span>
								</div>
			...
			$this->data['client'] = $this->Common_model->getData('client','',$feilds,'','','','','');
	...
	public function backBusiness(){
		if($_POST){
			...
			$feilds='COUNT(id) as id,SUM(point_in_rs) as point,SUM(winning_in_rs) as win,COUNT(DISTINCT customer_id) as cid';
			// $feilds='COUNT(id) as id,SUM(point) as point,SUM(winning_point) as win,COUNT(DISTINCT customer_id) as cid';

			$con1=' WHERE result_date BETWEEN "'.date('Y-m-d',strtotime($d[0])).'" AND "'.date('Y-m-d',strtotime($d[0])).'" AND regular_bazar_games.status NOT IN ("P","V") AND bazar_name IN ("3","4","5","6","7","10","12","14","16","17","18","20","21","22","26","27","28","68","69","70","73")';
	        $arr['inP'] = $this->Common_model->getData('regular_bazar_games',$con1,$feilds,'','','One','','');

	        $con2=' WHERE result_date BETWEEN "'.date('Y-m-d',strtotime($d[0])).'" AND "'.date('Y-m-d',strtotime($d[0])).'" AND regular_bazar_games.status NOT IN ("P","V") AND bazar_name IN ("9","13","15","23","24","25")';
	        $arr['outP'] = $this->Common_model->getData('regular_bazar_games',$con2,$feilds,'','','One','','');
	        
			$con3=' WHERE result_date BETWEEN "'.date('Y-m-d',strtotime($d[0])).'" AND "'.date('Y-m-d',strtotime($d[0])).'" AND regular_bazar_games.status NOT IN ("P","V") AND bazar_name IN ("8","11","19")';
	        $arr['inH'] = $this->Common_model->getData('regular_bazar_games',$con3,$feilds,'','','One','','');
	        
			$con=' INNER JOIN regular_bazar ON regular_bazar_games.bazar_name=regular_bazar.id WHERE result_date BETWEEN "'.date('Y-m-d',strtotime($d[0])).'" AND "'.date('Y-m-d',strtotime($d[0])).'" AND regular_bazar_games.status NOT IN ("P","V")';
			$feilds = 'COUNT(regular_bazar_games.id) as id,SUM(regular_bazar_games.point_in_rs) as point,COUNT(DISTINCT regular_bazar_games.customer_id) as cid,regular_bazar.bazar_name,regular_bazar.id as bazar_id,regular_bazar.open_time as oTime,regular_bazar.close_time as cTime,SUM(winning_in_rs) as win,SUM(point_in_rs-winning_in_rs) as ggr';
			// $feilds = 'COUNT(regular_bazar_games.id) as id,SUM(regular_bazar_games.point) as point,COUNT(DISTINCT regular_bazar_games.customer_id) as cid,regular_bazar.bazar_name,regular_bazar.id as bazar_id,regular_bazar.open_time as oTime,regular_bazar.close_time as cTime,SUM(winning_point) as win,SUM(point-winning_point) as ggr';
        	$arr['arr'] = $this->Common_model->getData('regular_bazar_games',$con,$feilds,'','','','regular_bazar.sequence asc','regular_bazar_games.bazar_name');

			$con4=' WHERE result_date BETWEEN "'.date('Y-m-d',strtotime($d[0])).'" AND "'.date('Y-m-d',strtotime($d[0])).'" AND regular_bazar_games.status NOT IN ("P","V")';
	        $arr['cUni'] = $this->Common_model->getData('regular_bazar_games',$con4,'COUNT(DISTINCT customer_id) as cUni','','','One','','');
	        // echo '<pre>';
			// print_r($arr['cUni']);
			// die();
	...
	public function segricatedRegularRoundStatmentData(){
		...
		$c = 0;
		if($_POST['resultDate']!=date('Y-m-d')){
			$con1 = " WHERE bazarName='".$_POST['bazar']."' AND resultDate='".$_POST['resultDate']."' AND type='".$_POST['gType']."'";
			$c = $this->Common_model->getDataForTest('cutting',$con1,'cuttingPercentage','','','One','','');
		}
		$nd = array_column($d, 'point', 'game');
		$nd1 = array_column($d, 'cId', 'game');
		$nd2 = array_column($d, 'uId', 'game');
		$arr=['status'=>200,'data'=>$nd,'dataCount'=>$nd1,'dataDistinct'=>$nd2,'pPercentage'=>$per,'cutting'=>$c];
		die(json_encode($arr));
	}
